...
class MyStreamListener(tweepy.StreamListener):
    ...
    def fight(self, Vitality, Spirit, Strength, Guard, Agility, Movement, Intellect, Wisdom, Skill, Luck, Moves, Items, Points, status):
        battling = 1
        playerchoice = 100
        if heroX == goalX and heroY == goalY:
            #Name, Vitality, Spirit, Strength, Guard, Agility, Movement, Intellect, Wisdom, Skill, Luck, Moves, Points
            enemylist = [["TEST_A", 50, 50, 50, 50, 50, 50, 50, 50, 5, 5, ["!!!"], 100]]
            enemy = choice(enemylist)
            enemyname = enemy[0]
            Display(self.heroname + " challenged the final boss: " + enemyname + "!")
        else:
            #Name, Vitality, Spirit, Strength, Guard, Agility, Movement, Intellect, Wisdom, Skill, Luck, Moves, Points
            enemylist = [["TEST_1", 11, 11, 11, 11, 11, 11, 11, 11, 1, 1, ["???"], 5],
                         ["TEST_2", 8, 20, 5, 5, 10, 10, 15, 20, 1, 0, ["Teleport", "Impair"], 8],
                         ["Viking", 13, 2, 20, 4, 11, 9, 5, 8, 10, 10, ["Charge", "Hype Up"], 10],
                         ["Wizard", 8, 20, 5, 5, 10, 10, 20, 15, 1, 5, ["Fireball", "Lightning", "Aqua Blast"], 10],
                         ["Mean Kid", 5, 5, 4, 4, 12, 15, 2, 1, 10, 2, ["Rock Throw", "Taunt", "Kick Dirt"], 7]
                        ]
            enemy = choice(enemylist)
            enemyname = enemy[0]
            Display(self.heroname + " encountered an enemy: " + enemyname + "!!")
        ...
        enemyhealth = enemy[1] * 10
        enemymagic = enemy[2] * 10
        enemyphysicalattack = enemy[3]
        enemyphysicaldefense = enemy[4]/2
        enemyspeed = enemy[5]
        enemyevasion = enemy[6]
        enemymagicattack = enemy[7]
        enemymagicdefense = enemy[8]/2
        enemyphysicalsecurity = enemy[9]
        enemymagicalsecurity = enemy[10]
        enemymoveset = enemy[11]
        enemyexperience = enemy[12]
        ...
        while health > 0 and enemyhealth > 0 and enemystance != "Fled":
            ...
            if enemyhealth > enemy[1]*10: enemyhealth = enemy[1]*10
            if enemymagic > enemy[2]*10: enemymagic = enemy[2]*10

            health = math.ceil(health)
            magic = math.ceil(magic)

            enemyhealth = math.ceil(enemyhealth)
            enemymagic = math.ceil(enemymagic)

            #Player Condition Effects
            if regeneration > 0 and health < Vitality * 10:
                health += Vitality * 0.1
                regeneration -= 1
                Display(self.heroname + " recovered " + str(Vitality * 0.1) + " health!\nCurrent HP: " + str(health))

            if burn > 0:
                damage = Vitality * 0.05 * fire_weakness
            ...
            if boint > 0:
                ...
                physicalattack = randint(1, 100)
                physicaldefense = randint(1, 50)
                speed = randint(1, 100)
                evasion = randint(1, 100)
                magicattack = randint(1, 100)
                magicdefense = randint(1, 50)

            #Enemy Condition Effects
            if enemyregeneration > 0 and enemyhealth < enemy[1] * 10:
                enemyhealth += enemy[1] * 0.1
                enemyregeneration -= 1
                Display(enemyname + " recovered " + str(enemy[1] * 0.1) + " health!")

            if enemyburn > 0:
                damage = enemy[1] * 0.05 * enemyfire_weakness
                damage = math.ceil(damage)
                enemyhealth -= damage
                burn -= 1
                Display(enemyname + " was hurt by the burn! It did " + str(damage) + " damage.")

            if enemyconfusion > 0:
                enemyconfusion -= 0
                Display(enemyname + " is confused!")

            if enemyparalysis > 0:
                enemyparalysis -= 1
                Display(enemyname + " is paralyzed!" )
            ...
            if enemypoison > 0:
                ...
                enemyhealth -= damage
                poison -= 1
                Display(enemyname + " was hurt by the poison! It did " + str(damage) + " damage.")

            if enemyrage > 0:
                enemyrage -= 1
                Display(enemyname + " is enraged!")

            if enemyboint > 0:
                Display(enemyname + " is boint!")
            ...
            if nextmove > speed:
                ...
                if enemyrage > 0: enemychoice = 1
                if enemyconfusion > 0: enemychoice = 3.14159265
                if enemyparalysis > 0: enemychoice = 0

                if enemychoice == 1:
                    #Enemy Attack
                    damage = (enemyphysicalattack + randint(enemyphysicalsecurity*-1, enemyphysicalsecurity)) - (physicaldefense)
                    damage = math.ceil(damage)
                    dodgechance = randint(1, 125)
                    if dodgechance > evasion and damage > 0:
                        health = health - damage
                        Display(enemyname + " attacks! " + str(damage) + " damage to " + self.heroname + ".\nCurrent health: " + str(health))
                    elif dodgechance <= evasion:
                        Display(enemyname + " attacks! " + self.heroname + " dodged the attack.\nCurrent health: " + str(health))
                    elif damage <= 0:
                        Display(enemyname + " attacks! " + self.heroname + " blocked the attack.\nCurrent health: " + str(health))
                    ...
                    if enemymove == "???":
                        ...
                        Display(enemyname + " examined " + self.heroname + ".\nHealth:" + str(health) + "/" + str(Vitality*10) + "\nMagic:" + str(magic) + "/" + str(Spirit*10) + "\nAttack:" + str(physicalattack) + "\nDefense:" + str(physicaldefense) + "\nSpeed:" + str(speed) + "\nEvasion:" + str(evasion) + "\nMagicAttack:" + str(magicattack) + "\nMagicDefense:" + str(magicdefense))
                    ...
                    #Buff
                        ...
                        if enemymagic >= 10:
                            ...
                            Display(enemyname + " hyped themselves up! Physical and magical stability decreased by 10%!" )
                            enemymagic -= 10
                        else:
                    ...
                    #Debuff
                        ...
                        if enemymagic >= 50:
                            ...
                            status = api.update_status(status=enemyname + " tried to impair " + self.heroname + ", but didn't have enough Magic..." )
                            print(enemyname + " tried to impair " + self.heroname + ", but didn't have enough Magic..." )
                    elif enemymove == "Taunt":
                        Display(enemyname + " called " + self.heroname + " bad names! This upset " + self.heroname + " immensely.")
                        rage += 5
                    elif enemymove == "Kick Dirt":
                        Display(enemyname + " kicked dirt in " + self.heroname + "'s eyes! Evasion reduced by 10%!")
                        evasion -= math.ceil(evasion * 0.1)
                    #Healing
                    elif enemymove == "Heal":
                        if enemymagic >= 20 and enemyhealth < enemy[1] * 10:
                            healing = enemymagicdefense*2 + enemymagicattack + randint(enemymagicalsecurity*-2, enemymagicalsecurity*2)
                            enemyhealth += healing
                    ...
                    #Plain
                        ...
                        if dodgechance > evasion:
                            ...
                            Display(enemyname + " charged forward! Dealt " + str(damage) + " and suffered " + str(physicaldefense) + " recoil damage!\nCurrent health: " + str(health) )
                        else:
                            enemyhealth -= (enemyphysicaldefense + enemyspeed)*0.8
                            Display(enemyname + " charged forward, but missed and fell on their face! Suffered " + str((enemyphysicaldefense + enemyspeed)*0.8) + " recoil damage!" )
                    #Elemental
                        ...
                        if enemymagic >= 50:
                            ...
                            if dodgechance > evasion and dodgechance % 2 == 0:
                                burn += 5
                                damage = (enemymagicattack*fire_weakness + randint(enemymagicalsecurity*-1, enemymagicalsecurity)) - (magicdefense)
                                damage = math.ceil(damage)
                                if damage <= 0: damage = 0
                                health -= damage
                                Display(enemyname + " launched a stream of flames! " + str(damage) + " damage to " + self.heroname + "! They also caught on fire.\nCurrent health: " + str(health) )
                            elif dodgechance > evasion and dodgechance % 2 == 1:
                                damage = (enemymagicattack*fire_weakness + randint(enemymagicalsecurity*-1, enemymagicalsecurity)) - (magicdefense)
                                if damage <= 0:
                                    damage = 1
                                health -= damage
                                Display(enemyname + " launched a stream of flames! " + str(damage) + " damage to " + self.heroname + "!\nCurrent health: " + str(health) )
                            elif dodgechance <= evasion:
                        ...
                        if enemymagic >= 25:
                            ...
                            if dodgechance > evasion:
                                ...
                                if damage < 0: damage = 0
                                health -= damage
                                Display(enemyname + " threw a fireball! " + str(damage) + " damage to " + self.heroname + "!" )
                            elif dodgechance <= enemyevasion:
                                Display(enemyname + " threw a fireball! " + self.heroname + " dodged it!" )
                            enemymagic -= 25
                        else:
                            Display(enemyname + " wanted to throw a fireball, but didn't have enough Magic..." )
                        ...
                        if dodgechance > evasion:
                            ...
                            if damage < 0: damage = 0
                            health -= damage
                            Display(enemyname + " threw a rock at " + self.heroname + ". It caused " + str(damage) + " damage. How rude!\nCurrent health: " + str(health) )
                        else:
                            Display(enemyname + " threw a rock at " + self.heroname + ", but it missed..." )
                    elif enemymove == "Lightning":
                        if enemymagic >= 25:
                            dodgechance = randint(1, 100)
                            if dodgechance > evasion and dodgechance % 5 == 0:
                                damage = (enemymagicattack*electric_weakness + randint(enemymagicalsecurity*-1, enemymagicalsecurity)) - (magicdefense)
                                damage = math.ceil(damage)
                                if damage < 0: damage = 0
                                health -= damage
                                paralysis += 5
                                Display(enemyname + " struck " +self.heroname + " with a bolt of lightning! " + str(damage) +" damage to " + self.heroname + "! They also became paralyzed." )
                            elif dodgechance > evasion and dodgechance % 2 == 1:
                                damage = (enemymagicattack * electric_weakness + randint(enemymagicalsecurity * -1,enemymagicalsecurity)) - (magicdefense)
                                if damage < 0: damage = 1
                                health -= damage
                                Display(enemyname + " struck " +self.heroname + " with a bolt of lightning! " + str(damage) +" damage to " + self.heroname + "!" )
                            else:
                                Display(enemyname + " tried to strike " + self.heroname + " with a bolt of lightning, but missed..." )
                            enemymagic -= 25
                        else:
                            status = api.update_status(status=enemyname + " tried to strike " + self.heroname + " with a bolt of lightning, but didn't have enough Magic..." )
                            print(enemyname + " tried to strike " + self.heroname + " with a bolt of lightning, but missed..." )
                    elif enemymove == "Leaf Barrage":
                        if enemymagic > 25:
                            dodgechance = randint(1, 200)
                            if dodgechance > evasion:
                                damage = (enemymagicattack*grass_weakness + randint(enemymagicalsecurity*-1, enemymagicalsecurity) - physicaldefense)
                                damage = math.ceil(damage)
                                if damage < 0: damage = 0
                                Display(enemyname + " shot sharp leaves at incredible speed! " + str(damage) + " damage to " + self.heroname + "." )
                            else:
                                Display(enemyname + " shot sharp leaves at incredible speed! They all missed..." )
                            enemymagic -= 25
                        else:
                            status = api.update_status(status=enemyname + " wanted to shoot sharp leaves at incredible speed, but didn't have enough Magic... " )
                            print(enemyname + " wanted to shoot sharp leaves at incredible speed, but didn't have enough Magic... " )
                    elif enemymove == "Gust":
                        if enemymagic >= 20:
                            dodgechance = randint(1, 135)
                            if dodgechance > evasion:
                                damage = enemymagicattack*wind_weakness + randint(enemymagicalsecurity*-1, enemymagicalsecurity) - (magicdefense)
                                damage = math.ceil(damage)
                                if damage < 0: damage = 0
                                health -= damage
                                Display(enemyname + " launched a gust of wind! " + str(damage) + " damage to " + self.heroname + "!" )
                            elif dodgechance <= evasion:
                        ...
                        if enemymagic > 20:
                            dodgechance = randint(1, 125)
                            if dodgechance > evasion:
                                damage = enemymagicattack*water_weakness + randint(enemymagicalsecurity*-1, enemymagicalsecurity) - (magicdefense)
                                damage = math.ceil(damage)
                                if damage < 0: damage = 0
                                health -= damage
                                Display(enemyname + " shot a jet of cold water! " + str(damage) + " damage to " + self.heroname + "!" )
                            elif dodgechance <= evasion:
                        ...
                        if enemymagic > 25:
                            dodgechance = randint(1, 200)
                            if dodgechance > evasion:
                                damage = (enemymagicattack*dark_weakness + randint(enemymagicalsecurity*-1, enemymagicalsecurity)) - (magicdefense)
                                damage = math.ceil(damage)
                                health =- damage
                                Display(enemyname + " engulfed " + self.heroname + " in darkness! " + str(damage) + " damage to " + self.heroname + "!" )
                            elif dodgechance <= evasion:
                                Display(enemyname + " engulfed " + self.heroname + " in darkness, but nothing happened?" )
                            enemymagic -= 25
                        else:
                            Display(enemyname + " wanted to engulf " + self.heroname + " in darkness, but didn't have enough Magic..." )
                    elif enemymove == "Holy Arrow":
                        if enemymagic >= 25:
                            dodgechance = randint(1, 200)
                            if dodgechance > evasion:
                                damage = (enemymagicattack*light_weakness + randint(enemymagicalsecurity*-1, enemymagicalsecurity)) - (magicdefense)
                                damage = math.ceil(damage)
                                health -= damage
                                Display(enemyname + " shot a radiant arrow. " + str(damage) + " damage to " + self.heroname + "!" )
                            elif dodgechance <= evasion:
                                Display(enemyname + " shot a radiant arrow at " + self.heroname + ", but it missed..." )
                            enemymagic -= 25
                        else:
                            Display(enemyname + " tried to shoot a radiant arrow at " + self.heroname + ", but didn't have enough Magic..." )
                ...
                if playerchoice == 1:
                    #Player Attack
                    damage = (physicalattack + randint(physicalsecurity*-1, physicalsecurity)) - (enemyphysicaldefense)
                    damage = math.ceil(damage)
                    dodgechance = randint(1, 125)
                    ...
                    if playermove == "Identify":
                        Display(self.heroname + " examined " + enemyname + ".\nHealth:" + str(enemyhealth) + "/" + str(enemy[1]*10) + "\nMagic:" + str(enemymagic) + "/" + str(enemy[2]*10) + "\nAttack:" + str(enemyphysicalattack) + "\nDefense:" + str(enemyphysicaldefense) + "\nSpeed:" + str(enemyspeed) + "\nEvasion:" + str(enemyevasion) + "\nMagicAttack:" + str(enemymagicattack) + "\nMagicDefense:" + str(enemymagicdefense) )
                    ...
                    #Healing
                        ...
                        if magic >= 20 and health < Vitality * 10:
                            healing = magicdefense*2 + magicattack + randint(magicalsecurity*-2, magicalsecurity*2)
                            health += healing
                            Display(self.heroname + " healed themselves; " + str(healing) + " HP gained!" )
                            magic -= 20
                        elif magic < 20:
                            status = api.update_status(status=self.heroname + " tried to heal themselves, but didn't have enough magic..." )
                            print(self.heroname + " tried to heal themselves, but didn't have enough magic." )
                        elif health >= Vitality * 10:
                            status = api.update_status(status=self.heroname + " tried to heal themselves, but was already at full health..." )
                            print(self.heroname + " tried to heal themselves, but already has full health..." )
                    #Plain
                    #Elemental
                    elif playermove == "Fireball":
                        if magic >= 25:
                            dodgechance = randint(1, 125)
                            if dodgechance > enemyevasion:
                                damage = (magicattack*enemyfire_weakness + randint(magicalsecurity * -1, magicalsecurity) - (enemymagicdefense))
                                damage = math.ceil(damage)
                                if damage < 0: damage = 0
                                enemyhealth -= damage
                                Display(self.heroname + " threw a fireball! " + str(damage) + " damage to " + enemyname + "!" )
                            elif dodgechance <= evasion:
    ...
    def on_status(self, status):
        ...
        if self.heroname == "Blank":
            ...
            Display("The hero's name shall be " + self.heroname + "!" + goal_announce)

        #Command outside battle
        elif self.heroname != "Blank" and battling == 0:
        ...
        #        Points -= amount
                ...
                if steps <= Movement:
                    if "right" in splitcommand[1]: self.heroX += steps
                    elif "left" in splitcommand[1]: self.heroX -= steps
                    elif "up" in splitcommand[1]: self.heroY += steps
                    elif "down" in splitcommand[1]: self.heroY -= steps
                    Display(self.heroname + " moved right " + str(steps) + " steps.\nCurrent position: (" + str(self.heroX) + "," + str(self.heroY) + ")")
                    moved = 1
                else:
        ...
        elif self.heroname != "Blank" and battling == 1:
